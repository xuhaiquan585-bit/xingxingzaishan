(function printBatchAdminModule() {
  ...
  function renderDetail() {
    ...
    el('printBatchQrTable').innerHTML = detail.qr_codes.map((qr) => `
      <tr>
        <td><input type="checkbox" data-print-qr-id="${qr.id}" ${state.selectedQrIds.has(qr.id) ? 'checked' : ''} /></td>
  ...
  function updateSelectionState() {
    const ids = state.detail ? state.detail.qr_codes.map((qr) => qr.id) : [];
    el('selectAllPrintQr').checked = ids.length > 0 && ids.every((id) => state.selectedQrIds.has(id));
    el('printVoidReasonRow').classList.toggle('hidden', state.selectedQrIds.size === 0);
  }
  ...
  function bind() {
    ...
    el('selectAllPrintQr').addEventListener('change', (event) => {
      ...
      if (event.target.checked) {
        state.detail.qr_codes.forEach((qr) => state.selectedQrIds.add(qr.id));
      }
      renderDetail();
    });
  }
  ...
  async function load() {
    bind();
    await run(async () => {
      await Promise.all([loadTemplates(), loadBatches()]);
    });
  }
