...
function readQrIssuanceAuthorityConfig(env = process.env) {
  const source = env && typeof env === 'object' ? env : {};
  const enabled = text(source.QR_ISSUANCE_POSTGRES_AUTHORITY_ENABLED);
  ...
  if (text(source.QR_ISSUANCE_POSTGRES_AUTHORITY_SCOPE) !== 'all') {
    return disabled('SCOPE_ALL_REQUIRED', { requested: true });
  }
  if (text(source.QR_ISSUANCE_POSTGRES_AUTHORITY_ALLOWLIST)) {
    return disabled('ALLOWLIST_FORBIDDEN', { requested: true });
  }

  const sourceHash = text(
    source.QR_ISSUANCE_POSTGRES_AUTHORITY_SOURCE_SHA256
  ).toLowerCase();
  if (!SOURCE_HASH_PATTERN.test(sourceHash)) {
    return disabled('SOURCE_SHA256_REQUIRED', { requested: true });
  }
  const domainHash = text(
    source.QR_ISSUANCE_POSTGRES_AUTHORITY_DOMAIN_SHA256
  ).toLowerCase();
