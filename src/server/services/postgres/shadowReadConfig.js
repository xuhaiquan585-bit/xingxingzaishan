...
function readStrictShadowConfig({
  env = process.env,
  enabledName,
  allowlistName,
  logDirectoryName,
  ...
  const enabledValue = env && env[enabledName];
  ...
  const allowlist = parseAllowlist(env[allowlistName]);
  if (!allowlist) return disabled('ALLOWLIST_REQUIRED');

  const rawLogDirectory = String(env[logDirectoryName] || '').trim();
