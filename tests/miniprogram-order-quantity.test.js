...
test('quantity limit uses product single-order limit and keeps 99 as the unlimited UI cap', () => {
  assert.equal(resolveQuantityLimit({ stock: 1 }), 1);
  assert.equal(resolveQuantityLimit({ stock: 10 }), 10);
  assert.equal(resolveQuantityLimit({ stock: 0 }), 99);
  assert.equal(resolveQuantityLimit({}), 99);
});
