...
test('user login pages should keep copy and expose miniapp-first login cues', () => {
  ...
  assert.equal(homeWxml.includes('以后重新扫码，还能看见。'), true);
  ...
  assert.equal(homeWxml.includes('区块链存证可验证'), true);
  ...
  assert.equal(homeWxss.includes('.home-memory-scene'), true);
  ...
  assert.equal(homeWxss.includes('#f8f6f1'), true);
  assert.equal(homeWxss.includes('#142844'), true);
  assert.equal(homeWxss.includes('#d5a84a'), true);
