...
test('admin page should expose section navigation and miniapp content tools', () => {
  ...
  assert.equal(html.includes('id="batchQrImagesExportBtn"'), false);
  assert.equal(html.includes('导出数据 CSV'), true);
  assert.equal(html.includes('创建印刷任务'), true);
  assert.equal(html.includes('历史二维码分类'), true);
  ...
  assert.equal(js.includes('/api/admin/records/qr-images/export'), false);
...
test('user login pages should keep copy and expose miniapp-first login cues', () => {
  ...
  const ordersWxss = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'orders', 'orders.wxss'), 'utf8');
  const orderDetailWxml = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'order-detail', 'order-detail.wxml'), 'utf8');
  const orderDetailJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'order-detail', 'order-detail.js'), 'utf8');
  const orderDetailWxss = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'order-detail', 'order-detail.wxss'), 'utf8');
  const miniappPaymentJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'utils', 'payment.js'), 'utf8');
  const projectWxml = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'project', 'project.wxml'), 'utf8');
  const projectJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'project', 'project.js'), 'utf8');
  const projectWxss = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'project', 'project.wxss'), 'utf8');
  const helpWxml = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'help', 'help.wxml'), 'utf8');
  const helpJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'help', 'help.js'), 'utf8');
  const helpWxss = fs.readFileSync(path.join(__dirname, '..', 'src', 'miniprogram', 'pages', 'help', 'help.wxss'), 'utf8');
  ...
  assert.equal(recordWxss.includes('.preview-panel .preview-secondary'), true);
  ...
  assert.equal(resultWxml.includes('mode="widthFix"'), true);
  assert.equal(resultWxml.includes('mode="aspectFit"'), false);
  ...
  assert.equal(recordDetailWxml.includes('mode="widthFix"'), true);
  assert.equal(recordDetailWxml.includes('mode="aspectFit"'), false);
  ...
  assert.equal(recordDetailWxss.includes('@import "../../styles/dawn.wxss"'), true);
  assert.equal(recordDetailWxss.includes('.detail-back-btn'), true);
  assert.equal(meWxml.includes('当前手机号：'), false);
  assert.equal(meWxml.includes('你留下的照片、文字和共创，都在这里'), true);
  ...
  assert.equal(homeWxml.includes('<button class="btn home-primary-cta" bindtap="goProducts">选择一颗星贴</button>'), true);
  assert.equal(homeWxml.includes('<button class="btn home-primary-cta" bindtap="scanCode">选择一颗星贴</button>'), false);
  assert.equal(homeWxml.includes('<button class="btn secondary home-scan-link" bindtap="scanCode">已有星贴？扫码记录</button>'), true);
  ...
  assert.equal(homeWxml.includes('{{content.home_subtitle}}'), true);
  assert.equal(homeWxml.includes('class="home-hero-image"'), true);
  ...
  assert.equal(homeWxml.includes('选择一颗星贴'), true);
  ...
  assert.equal(homeWxml.includes('class="home-section home-steps-section"'), true);
  assert.equal(homeWxml.includes('class="home-section home-memory-section"'), true);
  assert.equal(homeWxml.includes('class="home-section home-trust-section"'), true);
  assert.equal(homeWxml.includes('一张照片，一句话，就能留下'), true);
  assert.equal(homeWxml.includes('以下内容为产品使用示意'), true);
  assert.equal(homeWxml.includes('全部场景'), false);
  assert.equal(homeWxml.includes('区块链存证'), false);
  assert.equal(homeWxml.includes('NFT凭证'), false);
  assert.equal(homeWxml.includes('链上存证'), false);
  assert.equal(homeWxml.includes('区块链存证可验证'), false);
  ...
  assert.equal(homeJs.includes('content: normalizeHomeContent(data)'), true);
  ...
  assert.equal(homeWxss.includes('.home-page .home-scan-link'), true);
  assert.equal(homeWxss.includes('#f6f5f1'), true);
  assert.equal(homeWxss.includes('#142844'), true);
  assert.equal(homeWxss.includes('#d6a649'), true);
  assert.equal(homeWxss.includes('radial-gradient'), false);
  assert.equal(homeWxss.includes('@keyframes star-pulse'), false);
  assert.equal(homeWxss.includes('#020a14'), false);
  assert.equal(homeJson.includes('"navigationBarBackgroundColor": "#F6F5F1"'), true);
  assert.equal(homeJson.includes('"navigationBarTextStyle": "black"'), true);
  assert.equal(miniappAppJson.includes('"backgroundColor": "#FCFBF8"'), true);
  assert.equal(miniappAppJson.includes('"selectedColor": "#C89C4C"'), true);
  assert.equal(fs.statSync(homeHeroAsset).size < 350 * 1024, true);
  for (const iconName of ['home', 'seal', 'mine', 'about']) {
    const normalIcon = path.join(__dirname, '..', 'src', 'miniprogram', 'assets', 'tab', `${iconName}-normal.png`);
    const activeIcon = path.join(__dirname, '..', 'src', 'miniprogram', 'assets', 'tab', `${iconName}-active.png`);
  ...
  assert.equal(orderConfirmWxss.includes('@import "../../styles/dawn.wxss"'), true);
  ...
  assert.equal(ordersWxml.includes('bindtap="changeOrderFilter"'), true);
  assert.equal(ordersWxml.includes('bindtap="goProducts"'), true);
  assert.equal(ordersJs.includes("{ value: 'pending_payment', label: '待支付' }"), true);
  assert.equal(ordersJs.includes("{ value: 'paid', label: '待发货' }"), true);
  assert.equal(ordersJs.includes('applyOrderFilter'), true);
  assert.equal(ordersWxss.includes('@import "../../styles/dawn.wxss"'), true);
  assert.equal(orderDetailWxml.includes('订单详情'), true);
  assert.equal(orderDetailWxml.includes('lazy-load'), true);
  assert.equal(orderDetailWxml.includes('继续支付'), true);
  assert.equal(orderDetailWxml.includes('复制单号'), true);
  assert.equal(orderDetailJs.includes('payMiniappOrder'), true);
  assert.equal(orderDetailJs.includes('paying: false'), true);
  assert.equal(orderDetailJs.includes('wx.setClipboardData'), true);
  assert.equal(orderDetailJs.includes('maskPhone(order.receiver_phone)'), true);
  assert.equal(orderDetailWxss.includes('@import "../../styles/dawn.wxss"'), true);
  assert.equal(miniappPaymentJs.includes('wx.requestPayment'), true);
  assert.equal(miniappPaymentJs.includes('/api/miniapp/orders/${encodeURIComponent(orderId)}/pay'), true);
  assert.equal(miniappPaymentJs.includes("original.code = 'PAYMENT_CANCELLED'"), true);
  assert.equal(miniappPaymentJs.includes('account_id'), false);
  ...
  assert.equal(projectWxml.includes('{{content.project_body}}'), true);
  assert.equal(projectWxml.includes('{{content.brand_story_title}}'), true);
  assert.equal(projectWxml.includes('{{content.brand_story_body}}'), true);
  assert.equal(projectWxml.includes('星贴是用于记录的贴纸，不含酒水'), true);
  assert.equal(projectWxml.includes('bindtap="goHelp"'), true);
  assert.equal(projectWxml.includes('bindtap="openPrivacy"'), true);
  assert.equal(projectWxml.includes('open-type="contact"'), true);
  assert.equal(projectJs.includes('wx.openPrivacyContract'), true);
  assert.equal(projectJs.includes('consult_url'), false);
  assert.equal(projectWxss.includes('@import "../../styles/dawn.wxss"'), true);
  assert.equal(miniappAppJson.includes('pages/help/help'), true);
  assert.equal(helpWxml.includes('星贴、记录、共创和订单的常见问题'), true);
  assert.equal(helpWxml.includes('open-type="contact"'), true);
  assert.equal(helpWxml.includes('bindtap="openPrivacy"'), true);
  assert.equal(helpJs.includes('wx.openPrivacyContract'), true);
  assert.equal(helpJs.includes('web-view'), false);
  assert.equal(helpWxss.includes('@import "../../styles/dawn.wxss"'), true);
  assert.equal(meWxml.includes('我的星星'), true);
  assert.equal(meWxml.includes('我的订单'), true);
  assert.equal(meWxml.includes('使用帮助'), true);
  assert.equal(meWxml.includes('隐私保护指引'), true);
  assert.equal(meWxml.includes('open-type="contact"'), true);
  assert.equal(miniappMeJs.includes('wx.openPrivacyContract'), true);
  assert.equal(miniappMeJs.includes('/pages/help/help'), true);
  assert.equal(meWxml.includes('bindtap="openRecord"'), true);
  assert.equal(meWxml.includes('class="record-meta-group"'), true);
  assert.equal(meWxml.includes('class="dawn-state me-state"'), true);
...
test('POST /api/admin/records/qr-images/export is retired as a formal production bypass', async () => {
  const login = await postJson('/api/admin/login', { username: 'admin', password: 'test-admin-pass' });
  const token = login.body.data.token;
  ...
  assert.equal(response.body.code, 'LEGACY_QR_IMAGE_EXPORT_RETIRED');
  assert.match(response.body.message, /印刷任务/u);
});
