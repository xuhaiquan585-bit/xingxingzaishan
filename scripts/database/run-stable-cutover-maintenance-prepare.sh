...
latest_joint_summary() {
  ...
  while IFS= read -r file; do
    if grep -qx 'CLEAN_CANDIDATE_COORDINATED_JOINT_REHEARSAL=PASS' "$file" &&
       grep -qx 'POSTGRES_ONLY_BATCH_ADMINISTRATION=PASS' "$file" &&
       grep -qx 'POSTGRES_ONLY_QUALITY_CHECK=PASS' "$file" &&
       grep -qx 'POSTGRES_ONLY_ADMIN_VISIBILITY=PASS' "$file" &&
       grep -qx 'POSTGRES_ONLY_NFT_READ=PASS' "$file" &&
