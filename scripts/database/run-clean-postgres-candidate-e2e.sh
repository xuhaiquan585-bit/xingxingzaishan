...
cleanup() {
  ...
  if [ "$original_status" -eq 0 ] && [ "$cleanup_status" -eq 0 ] && \
    ...
    printf '%s\n' \
      ...
      'POSTGRES_ONLY_BATCH_ADMINISTRATION=PASS' \
      'POSTGRES_ONLY_QUALITY_CHECK=PASS' \
      'POSTGRES_ONLY_ADMIN_VISIBILITY=PASS' \
      'POSTGRES_ONLY_NFT_READ=PASS' \
...
grep -q 'CLEAN_CANDIDATE_POSTGRES_ONLY_BATCH_ADMINISTRATION=PASS' "$TEST_LOG"
grep -q 'CLEAN_CANDIDATE_POSTGRES_ONLY_QUALITY_CHECK=PASS' "$TEST_LOG"
grep -q 'CLEAN_CANDIDATE_POSTGRES_ONLY_ADMIN_VISIBILITY=PASS' "$TEST_LOG"
grep -q 'CLEAN_CANDIDATE_POSTGRES_ONLY_NFT_READ=PASS' "$TEST_LOG"
