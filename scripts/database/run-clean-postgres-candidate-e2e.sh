...
cleanup() {
  ...
  if [ "$original_status" -eq 0 ] && [ "$cleanup_status" -eq 0 ] && \
    ...
    printf '%s\n' \
      ...
      'POSTGRES_ONLY_PROOF_WORKER=PASS' \
    ...
    echo 'NEXT_ACTION=PREPARE_COORDINATED_JOINT_REHEARSAL'
    exit 0
  fi
...
export RECORD_PROOF_RUNTIME_ENABLED=true
export RECORD_PROOF_RUNTIME_SCOPE=all
export RECORD_PROOF_RUNTIME_SOURCE_SHA256="$EXPECTED_SOURCE_SHA"
export RECORD_PROOF_RUNTIME_DOMAIN_SHA256="$EXPECTED_DOMAIN_SHA"
export RECORD_PROOF_WORKER_ID=clean-candidate-e2e-worker
export MINIAPP_MOCK_ENABLED=true
export CHAIN_ENABLED=true
export CHAIN_CALLBACK_URL=https://clean-candidate.invalid/api/chain/avata/callback
export AVATA_API_KEY=clean-candidate-e2e-key
export AVATA_API_SECRET=clean-candidate-e2e-secret
export AVATA_IDENTITY_NAME=clean-candidate-e2e-name
export AVATA_IDENTITY_NUM=clean-candidate-e2e-number
...
grep -q 'CLEAN_CANDIDATE_POSTGRES_ONLY_PROOF_WORKER=PASS' "$TEST_LOG"
