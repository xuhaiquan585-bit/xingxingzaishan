#!/usr/bin/env bash
set -Eeuo pipefail
umask 077

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
SOURCE=/root/public-qr-domain-marker-audit-20260811/production-db-source.json
EXPECTED_SOURCE_SHA=f263df13b5c19f91b0f86d93960f6b26896f3ed605318c73dd8546d110b06cfd
EXPECTED_CANDIDATE_SHA=93def24ee6dd4de63fd4ebf776a0a2056d2563df492727231b8f6de08ec0c7ee
EXPECTED_CANDIDATE_DOMAIN_SHA=be64b9b040d8b188b8bae9fb63e87621263bca9d8b76d40bf8c8ed302f08fa9d
EXPECTED_EXCLUDED_QR_IDS=STAR0001
EXPECTED_PRIVACY_QR_IDS=SSS00003,SSS00008,SSS00009
PREPARATION_ROOT=/root/legacy-privacy-remediation-preparation-audit-20260812
AUDIT_ROOT=/root/clean-postgres-baseline-plan-audit-20260812
RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$$"
AUDIT_DIR="$AUDIT_ROOT/$RUN_ID"
REPORT="$AUDIT_DIR/clean-baseline-plan.json"

fail() {
  echo "CLEAN_POSTGRES_BASELINE_PLAN=BLOCKED_$1" >&2
  exit 1
}

assert_default_off() {
  local app_pid="$1"
  local flag value
  for flag in \
    PUBLIC_QR_SHADOW_READ_ENABLED \
    PERSONAL_RECORD_SHADOW_READ_ENABLED \
    IDENTITY_SHADOW_READ_ENABLED \
    PUBLIC_QR_POSTGRES_READ_ENABLED \
    QR_LIFECYCLE_POSTGRES_WRITE_ENABLED \
    RECORD_PROOF_RUNTIME_ENABLED \
    PERSONAL_RECORD_POSTGRES_READ_ENABLED
  do
    tr '\0' '\n' < "/proc/$app_pid/environ" |
      grep -qx "${flag}=false" || return 1
  done
  for flag in IDENTITY_POSTGRES_AUTHORITY_ENABLED QR_ISSUANCE_POSTGRES_AUTHORITY_ENABLED
  do
    value="$(
      tr '\0' '\n' < "/proc/$app_pid/environ" |
        sed -n "s/^${flag}=//p" |
        tail -n 1
    )"
    [ -z "$value" ] || [ "$value" = false ] || return 1
  done
  if tr '\0' '\n' < "/proc/$app_pid/environ" |
     grep -Eq '^(DATABASE_URL|PGPASSWORD|AVATA_API_KEY|AVATA_API_SECRET)=.+$'; then
    return 1
  fi
}

[ "$(id -u)" -eq 0 ] || fail ROOT_REQUIRED
cd "$REPO"
[ -z "$(git status --porcelain --untracked-files=no)" ] || fail TRACKED_WORKTREE_DIRTY
[ -f "$SOURCE" ] && [ ! -L "$SOURCE" ] || fail SOURCE_INVALID
[ "$(stat -c '%U:%G' "$SOURCE")" = root:root ] || fail SOURCE_OWNER_INVALID
[ "$(stat -c '%a' "$SOURCE")" = 600 ] || fail SOURCE_MODE_INVALID
[ "$(sha256sum "$SOURCE" | awk '{print $1}')" = "$EXPECTED_SOURCE_SHA" ] || \
  fail SOURCE_HASH_MISMATCH

mapfile -t CANDIDATES < <(
  find "$PREPARATION_ROOT" -mindepth 2 -maxdepth 2 \
    -type f -name candidate-db.json -print0 |
    xargs -0 -r sha256sum |
    awk -v expected="$EXPECTED_CANDIDATE_SHA" '$1 == expected { print $2 }'
)
[ "${#CANDIDATES[@]}" -eq 1 ] || fail CANDIDATE_NOT_UNIQUE
CANDIDATE="${CANDIDATES[0]}"
PREPARATION_REPORT="$(dirname "$CANDIDATE")/preparation-report.json"
[ -f "$PREPARATION_REPORT" ] && [ ! -L "$PREPARATION_REPORT" ] || \
  fail PREPARATION_REPORT_INVALID
for FILE in "$CANDIDATE" "$PREPARATION_REPORT"; do
  [ "$(stat -c '%U:%G' "$FILE")" = root:root ] || fail ARTIFACT_OWNER_INVALID
  [ "$(stat -c '%a' "$FILE")" = 600 ] || fail ARTIFACT_MODE_INVALID
done

APP_PID="$(pm2 pid xingxingzaishan | tail -n 1)"
[ -n "$APP_PID" ] && [ "$APP_PID" != 0 ] || fail APP_PID_MISSING
HTTP_CODE="$(curl -sS -o /dev/null -w '%{http_code}' \
  --connect-timeout 5 --max-time 10 http://127.0.0.1:3000/)"
[ "$HTTP_CODE" = 200 ] || fail APP_HTTP_INVALID
assert_default_off "$APP_PID" || fail RUNTIME_NOT_DEFAULT_OFF
if ss -tnp | grep ':5432' | grep -Fq "pid=$APP_PID,"; then
  fail POSTGRES_RUNTIME_CONNECTION_PRESENT
fi

install -d -o root -g root -m 0700 "$AUDIT_ROOT" "$AUDIT_DIR"
install -o root -g root -m 0600 /dev/null "$REPORT"

unset DATABASE_URL PGHOST PGPORT PGUSER PGPASSWORD PGDATABASE
unset PGSSL PGSSLMODE PGOPTIONS PGAPPLICATION_NAME
unset AVATA_API_KEY AVATA_API_SECRET AVATA_ENV AVATA_API_BASE CHAIN_ENABLED

/usr/local/bin/node scripts/database/plan-clean-postgres-baseline.js \
  --plan-only \
  --source="$SOURCE" \
  --candidate="$CANDIDATE" \
  --preparation-report="$PREPARATION_REPORT" \
  --expected-source-sha256="$EXPECTED_SOURCE_SHA" \
  --expected-candidate-sha256="$EXPECTED_CANDIDATE_SHA" \
  --expected-candidate-domain-sha256="$EXPECTED_CANDIDATE_DOMAIN_SHA" \
  --exclude-qr-ids="$EXPECTED_EXCLUDED_QR_IDS" \
  --retained-privacy-qr-ids="$EXPECTED_PRIVACY_QR_IDS" \
  > "$REPORT"

/usr/local/bin/node - "$REPORT" <<'NODE'
const assert = require('node:assert/strict');
const fs = require('node:fs');
const report = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));
assert.equal(report.mode, 'plan-only');
assert.equal(report.status, 'READY');
assert.equal(report.strategy, 'CLEAN_POSTGRES_BASELINE_FROM_PRIVACY_CANDIDATE');
assert.deepEqual(report.excluded_qr_ids, ['STAR0001']);
assert.deepEqual(report.retained_privacy_remediation_qr_ids, [
  'SSS00003', 'SSS00008', 'SSS00009'
]);
assert.equal(report.candidate_counts.qr_codes, 104);
assert.equal(report.target_counts.qr_codes, 103);
assert.equal(report.removed_counts.qr_codes, 1);
assert.equal(report.target_import_status, 'READY');
assert.deepEqual(report.target_blocked_reasons, []);
assert.equal(report.target_privacy_finding_count, 0);
assert.equal(report.production_database_access, 'NONE');
assert.equal(report.production_database_write, 'NONE');
assert.equal(report.production_json_write, 'NONE');
assert.equal(report.oss_access, 'NONE');
assert.equal(report.external_provider_calls, 'NONE');
assert.equal(report.target_baseline_persisted, false);
assert.equal(report.rollback_requirements.current_postgres_backup_required_before_apply, true);
assert.equal(report.rollback_requirements.live_json_backup_required_before_apply, true);
console.log('CLEAN_POSTGRES_BASELINE_REPORT_GATE=PASS');
console.log(`TARGET_SOURCE_SHA256=${report.target_source_sha256}`);
console.log(`TARGET_PLAN_SHA256=${report.target_plan_sha256}`);
console.log(`TARGET_DOMAIN_SHA256=${report.target_public_qr_domain_sha256}`);
console.log(`TARGET_QR_COUNT=${report.target_counts.qr_codes}`);
console.log(`TARGET_RECORD_COUNT=${report.target_counts.records}`);
console.log(`REMOVED_COUNTS=${JSON.stringify(report.removed_counts)}`);
NODE

[ "$(sha256sum "$SOURCE" | awk '{print $1}')" = "$EXPECTED_SOURCE_SHA" ] || \
  fail SOURCE_CHANGED
[ "$(sha256sum "$CANDIDATE" | awk '{print $1}')" = "$EXPECTED_CANDIDATE_SHA" ] || \
  fail CANDIDATE_CHANGED
APP_PID_AFTER="$(pm2 pid xingxingzaishan | tail -n 1)"
[ "$APP_PID_AFTER" = "$APP_PID" ] || fail RUNTIME_RESTARTED
if ss -tnp | grep ':5432' | grep -Fq "pid=$APP_PID_AFTER,"; then
  fail POSTGRES_RUNTIME_CONNECTION_CREATED
fi

stat -c 'REPORT_OWNER=%U:%G MODE=%a SIZE=%s PATH=%n' "$REPORT"
echo "REPORT_SHA256=$(sha256sum "$REPORT" | awk '{print $1}')"
echo "VALIDATED_HEAD=$(git rev-parse HEAD)"
echo "APP_PID=$APP_PID_AFTER"
echo "APP_HTTP=$HTTP_CODE"
echo 'CLEAN_POSTGRES_BASELINE_PLAN=PASS'
echo 'PRODUCTION_DATABASE_ACCESS=NONE'
echo 'PRODUCTION_JSON_WRITE=NONE'
echo 'OSS_ACCESS=NONE'
echo 'EXTERNAL_PROVIDER_CALLS=NONE'
echo 'PRODUCTION_RUNTIME_RESTARTED=NO'
echo 'NEXT_ACTION=REVIEW_CLEAN_BASELINE_PLAN'
