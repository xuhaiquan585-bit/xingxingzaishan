...
grep -qx 'POSTGRES_ONLY_BATCH_ADMINISTRATION=PASS' "$JOINT_SUMMARY" || \
  fail JOINT_BATCH_ADMINISTRATION_NOT_PASSED
grep -qx 'POSTGRES_ONLY_QUALITY_CHECK=PASS' "$JOINT_SUMMARY" || \
  fail JOINT_QUALITY_CHECK_NOT_PASSED
grep -qx 'POSTGRES_ONLY_ADMIN_VISIBILITY=PASS' "$JOINT_SUMMARY" || \
  fail JOINT_ADMIN_VISIBILITY_NOT_PASSED
grep -qx 'POSTGRES_ONLY_NFT_READ=PASS' "$JOINT_SUMMARY" || \
  fail JOINT_NFT_READ_NOT_PASSED
...
[ "$CONFIG_STATUS" = READY_FOR_POSTGRES_MAINTENANCE_WINDOW ] || \
  fail CONFIG_STATUS_MISMATCH
...
PREFLIGHT_STATUS=READY_FOR_POSTGRES_MAINTENANCE_WINDOW
NEXT_ACTION=REVIEW_BACKUPS_AND_PREPARE_POSTGRES_CUTOVER_RUNNER
...
printf '%s\n' \
  ...
  'JOINT_QR_ADMINISTRATION_GATE=PASS' \
  'POSTGRES_SCOPE_ALL_CONFIG_GATE=PASS' \
  'STATIC_ALLOWLIST_COUNT=0' \
  'OUTBOX_BACKLOG_COUNT=0' \
  'AVATA_MIGRATION_SCOPE=DEFERRED' \
  'RECORD_PROOF_RUNTIME_ENABLED=false' \
